...
function FavoritesPage() {
  ...
  const handleToggleFavorite = async (noteId) => {
    ...
    try {
        ...
        if(!response.data.isFavorite) {
            setRawFavorites(prev => prev.filter(f => f.noteId !== noteId));
        }
    } catch (err) {
        console.error(err);
    }
  ...
  const FilterContent = () => (
    <div className="space-y-8">
        ...
        <div className="space-y-3 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
          {categories.map(cat => (
            <label key={cat.id} className="flex items-center space-x-3 cursor-pointer group">
              ...
              />
              <span className="text-gray-700 font-medium group-hover:text-rose-500 transition-colors">{cat.name}</span>
            </label>
          ))}
        </div>
      </div>
      ...
      <div className="border-t border-gray-100 pt-6">
        <h3 className="font-black text-gray-800 mb-4 uppercase tracking-wider text-sm">Added</h3>
          ...
          {[
            ...
            <label key={option.value} className="flex items-center space-x-3 cursor-pointer group">
              <input 
                ...
                className="w-5 h-5 border-gray-300 text-rose-500 focus:ring-rose-500 cursor-pointer"
              />
              <span className="text-gray-700 font-medium group-hover:text-rose-500 transition-colors">{option.label}</span>
            </label>
          ))}
        </div>
      </div>
  ...
  const unownedFavorites = filteredFavorites.filter(fav => !libraryIds.includes(fav.noteId));
  const ownedFavorites = filteredFavorites.filter(fav => libraryIds.includes(fav.noteId));
    ...
    <div className="max-w-7xl mx-auto py-8 px-4 relative min-h-screen">
      ...
      <div className="flex justify-between items-center mb-8 border-b border-gray-200 pb-4">
        <div>
            ...
            <p className="text-gray-500 font-medium mt-1">Notes you have saved for later or purchased.</p>
        </div>
        <span className="bg-rose-100 text-rose-800 font-bold px-4 py-2 rounded-lg">
          {rawFavorites.length} Saved Notes
        </span>
      </div>
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center bg-white p-4 rounded-2xl shadow-sm border border-gray-100 mb-6 gap-4">
              ...
              <form onSubmit={handleSearchSubmit} className="w-full relative">
                <input 
                  ...
                  className="w-full pl-5 pr-12 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-rose-500 outline-none font-medium"
                />
            ...
            {filteredFavorites.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed border-gray-300">
                ...
                <button 
                  onClick={clearFilters}
                  className="mt-6 px-6 py-3 bg-rose-500 text-white font-bold rounded-xl hover:bg-rose-600 transition-colors shadow-sm"
                >
                  Clear Filters
                </button>
              </div>
              ...
              <div className="space-y-12">
                ...
                <div>
                    ...
                    {unownedFavorites.length === 0 ? (
                        <p className="text-gray-500 italic">No matches in your unowned favorites.</p>
                    ) : (
                ...
                <div>
                    ...
                    {ownedFavorites.length === 0 ? (
                        <p className="text-gray-500 italic">No matches in your purchased favorites.</p>
                    ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
